...
/// @title IMT - Incremental Merkle Tree
/// @notice Implements an incremental Merkle tree with Poseidon hashing for zero-knowledge applications
contract IMT {
    /// @notice Depth of the Merkle tree
    uint32 public immutable i_depth;

    /// @notice Poseidon hasher instance
    Poseidon2 public immutable i_hasher;

    /// @notice Stores recent roots of the tree
    mapping(uint32 => bytes32) public s_roots;

    /// @notice Number of historical roots to store
    uint32 public constant ROOT_HISTORY_SIZE = 30;

    /// @notice Index of the most recent root
    uint32 public s_currentRootIndex;

    /// @notice Index of the next leaf to be inserted
    uint32 public s_nextLeafIndex;

    /// @notice Caches intermediate subtrees for efficient updates
    mapping(uint32 => bytes32) public s_cacheSubtrees;

    /// @dev Errors for invalid depth or full tree
    ...
    /// @param _depth depth of the Merkle tree (must be >0 and <32)
    ...
    /// @notice Returns precomputed zero value for a given level
    /// @param i tree level
    ...
    /// @notice Checks if a root is part of the recent history
